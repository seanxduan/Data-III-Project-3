## Question - How accurately can the outcome of a pass play be predicted?

Completion percentage, which is the ratio of completed passes to total number of passes attempted, is considered one of the most important statistics used to evaluate a quarterback’s performance. Of interest is predicting whether a pass is completed or not given other information about the play. This could possibly yield valuable information about which kinds of plays are easiest for the secondary to defend against.

Before performing any data analysis, the average weight, height, and age variables were converted to variables for the differences in weight, height, and age between the offense and the defense. In addition, passResult was converted to a binary response by removing plays in which a sack occurred and counting interceptions as incomplete passes. Not counting interceptions as incomplete passes would result in a class imbalance that could affect model fitting. Predicting interceptions was also seen as adding little value to the analysis. The objective is to predict whether passResult is “I” or “C” after making these modifications to the data. offensePlayResult and epa were not used as predictors, as including them would not yield any valuable information (i.e., it is highly likely that a pass was incomplete knowing the offense gained 0 yards on the play).

Models that were considered include: logistic regression, L1 penalized logistic regression, L2 penalized logistic regression, linear discriminant analysis, K-nearest neighbors, CART, bagging, boosting, random forest, support vector machines, and neural networks. All models were fit using the training set and then evaluated using the test set, with classification accuracy being the performance metric used. Quadratic discriminant analysis was attempted as well, but rank deficiency posed a problem for fitting this model. Parameters for all models were selected using 10-fold cross-validation.

All of these models are very commonly used in the case of a binary classification problem. Below is a table containing the test classification rate for all 14 models.

![](/Users/davidreynolds/Downloads/Documents/GitHub/Data-III-Project-3/Images/model_performance.jpg)

It is disappointing that a more flexible model did not perform better than logistic regression, and it is interesting that the test classification rates are so close to the percentage of completed passes in the training and test sets, which is about 65%. The classification rates are only slightly better than random guessing, so it appears that the outcome of a pass play cannot be predicted very accurately using these covariates.

Below is a variable importance plot for the best tree model, which was boosting. The importance scores for type of dropback and number of pass rushers were high for both boosting and the classification tree. The importance score for third down was high for both boosting and the classification tree as well. In comparison, the total time remaining in the game and player attributes such as age, weight, and height had high importance scores for both bagging and random forest.

![](/Users/davidreynolds/Downloads/Documents/GitHub/Data-III-Project-3/Images/variable_importance.jpg)

```{r moresetup, eval = FALSE}
library(caret)
library(DT)
library(dplyr)
library(e1071)
